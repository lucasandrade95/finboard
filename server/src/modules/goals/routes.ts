...
import { goalDocs } from '../../docs/schemas.js'
...
export function registerGoalRoutes(app: FastifyInstance, repository: GoalsRepository): void {
  ...
  app.get('/api/goals', { ...protectedRoute, schema: goalDocs.list }, async (request) => ({
    items: repository.list(ownerId(request)),
  }))

  app.post('/api/goals', { ...protectedRoute, schema: goalDocs.create }, async (request, reply) => {
  ...
  app.put(
    '/api/goals/:id',
    { ...protectedRoute, schema: goalDocs.update },
    async (request, reply) => {
  ...
  app.post(
    ...
    { ...protectedRoute, schema: goalDocs.contribute },
    async (request, reply) => {
      const { id } = idParamSchema.parse(request.params)
  ...
  app.delete(
    '/api/goals/:id',
    { ...protectedRoute, schema: goalDocs.remove },
    async (request, reply) => {
      const { id } = idParamSchema.parse(request.params)
