...
import { budgetDocs } from '../../docs/schemas.js'
...
export function registerBudgetRoutes(app: FastifyInstance, repository: BudgetsRepository): void {
  ...
  app.get('/api/budgets', { ...protectedRoute, schema: budgetDocs.list }, async (request) => {
  ...
  app.put(
    ...
    { ...protectedRoute, schema: budgetDocs.upsert },
    async (request) => {
  ...
  app.delete(
    '/api/budgets/:category',
    { ...protectedRoute, schema: budgetDocs.remove },
    async (request, reply) => {
      const { category } = budgetCategoryParamSchema.parse(request.params)
